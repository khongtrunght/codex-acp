...
async function main() {
  ...
  try {
    ...
    await withTimeout(conn.unstable_closeSession({ sessionId: session.sessionId }), 10000, 'close original session');
    await withTimeout(conn.unstable_closeSession({ sessionId: forked.sessionId }), 10000, 'close fork session');
